---
ms.openlocfilehash: c591478de78ed33ac02839eae08d756294d47a77
ms.sourcegitcommit: 28b5b81155de28693455114a5fc5941cb314c9ef
ms.translationtype: HT
ms.contentlocale: ar-SA
ms.lasthandoff: 01/31/2022
ms.locfileid: "8075777"
---
...
14. أنشئ ملف JSON، والصقه في التعليمات البرمجية التالية، ثم احفظه.
    ...
    ```
15. إلى جانب حقل **عينة البيانات**، حدد **اختيار ملف** وحمّل ملف JSON الذي أنشأته.
16. حدد **حفظ وإغلاق**.

## <a name="add-provider-definition-transformation-to-solution"></a>إضافة تحويل تعريف الموفر إلى حل

لإضافة تحويل تعريف الموفر إلى حل، اتبع الخطوات التالية.

1. انتقل إلى [مدخل منشئ Power App](https://make.powerapps.com) وانتقل إلى الحل الذي تم إنشاؤه حديثاً **LabProviders**.
1. حدد **إضافة موجود \> تحويل تعريف الموفر**.
1. حدد **تمرين عملي - أمر إلى أمر Dataverse**، ثم حدد **إضافة‏‎** لإضافته إلى الحل.

## <a name="create-provider-message-handler"></a>إنشاء معالج رسائل الموفر

لإنشاء معالج رسائل الموفر، اتبع الخطوات التالية.

1. انتقل إلى [مدخل منشئ Power App](https://make.powerapps.com)، وانتقل إلى **الحلول**، وافتح **الحل الافتراضي**. 
1. حدد **جديد**.
...
1. في **مع المرفق فقط**، حدد **نعم‏‎**.

    ![لقطة شاشة للصفحة "مشغّل Microsoft Outlook".](../media/outlook-trigger-ss.png)
...
1. في **المحتوى**، الصق التعليمات البرمجية التالية:
    ...
    ```
1. حدد **إنشاء من عينة**، والصق عينة التعليمات البرمجية التالية للمخطط:
    ```JSON
    {
      "type": "object",
      "properties": {
    ...
    ```
1. حدد **تم**.
1. حدد علامة القطع ("**...**") ثم حدد **إعادة تسمية**. 
1. غيّر اسم الإجراء إلى **خصائص نظام IOM**.
1. حدد **حفظ**.

    ![لقطة شاشة لصفحة الإجراء "خصائص النظام"](../media/system-properties-ss.png)
1. حدد **خطوة جديدة**، وأضف إجراء **تحليل JSON**، وغيّر اسمه إلى **تهيئة متغيرات الموفر**.
1. في **المحتوى**، الصق التعليمات البرمجية التالية:
    ```JSON
    {
    ...
    ``` 
1. حدد **حفظ**.
1. حدد **خطوة جديدة**، وأضف إجراء **تهيئة المتغير**، وغيّر اسمه إلى **تهيئة معالجة نتيجة التنفيذ**.
...
1. حدد **خطوة جديدة**، وأضف "النطاق"، وغيّر اسمه إلى **تجربة**.
1. في النطاق **تجربة**، حدد **إضافة إجراء**.
1. أضف "تنفيذ إجراء غير مرتبط" من موصل **Dataverse** وغيّر اسمه إلى **إقرار باستلام رسالة الموفر**.
...
1. حدد **حفظ**.
    ...
    ![لقطة شاشة للصفحة "إقرار باستلام رسالة الموفر".](../media/acknowledge-provider-message-ss.png)
1. حدد **إضافة إجراء**، ثم أضف عنصر تحكم **تطبيق على كل**.
1. في **حدد إخراجاً من الخطوات السابقة**، حدد **المرفقات**.
1. حدد **إضافة إجراء** داخل الحلقة **تطبيق على كل**، وأضف **تشغيل سير عمل تابع‬** من موصل **سير العمل**، وغيّر اسمه إلى **تحويل الرسالة بواسطة Power Query Online**.
...
1. حدد **حفظ**.
    ...
    ![لقطة شاشة للصفحة "تحويل الرسالة بواسطة Power Query."](../media/transform-message-power-ss.png)
1. بعد خطوة التحويل، حدد **إضافة إجراء**، وأضف **تشغيل سير عمل تابع** من موصل سير العمل، وغيّر اسمه إلى **إنشاء أمر**.
1. في **سير عمل تابع**، أدخل **إنشاء أمر عمل**.
...
1. في النطاق **التقاط**، حدد علامة القطع ("**...**")، ثم حدد **تكوين التشغيل بعد**.
1. حدد خانتي الاختيار **فشل** و **انتهت مهلته**، ثم حدد **تم**.

    ![لقطة شاشة لصفحة "تكوين التشغيل بعد".](../media/run-after-ss.png)
1. في النطاق **التقاط**، حدد **إضافة إجراء**، وأضف الإجراء **تعيين المتغير** وغيّر اسمه إلى **تعيين نتيجة التنفيذ إلى فاشلة**.
1. في **الاسم**، أدخل **Executionresult**.
1. في **القيمة**، حدد **خطأ**.
1. حدد **حفظ**.
...
1. حدد **خطوة جديدة**، وأضف نطاقاً، وغيّر اسمه إلى **أخيراً**.
1. في النطاق **أخيراً**، حدد علامة القطع ("**...**")، ثم حدد **تكوين التشغيل بعد**. 
1. حدد خانات الاختيار **هو ناجح** و **فشل‏‎** و **تم تخطيه** و **انتهت مهلته**، ثم حدد **تم**.

    ![تكوين التشغيل بعد (لقطة شاشة لصفحة النطاق "أخيراً)](../media/configure-run-after-2-ss.png)
1. في النطاق **أخيراً**، حدد **إضافة إجراء**، وأضف إجراء **تنفيذ إجراء غير مرتبط** وغيّر اسمه إلى **نتيجة تنفيذ طلب حفظ رسائل الموفر**.
...
1. حدد **حفظ**.
    ...
    ![لقطة شاشة لنتيجة تنفيذ طلب حفظ رسائل الموفر‬‏‫.](../media/save-provider-message-request-ss.png)

## <a name="add-provider-definition-logic-definition-to-the-provider-definition"></a>إضافة تعريف منطق تعريف الموفر إلى تعريف الموفر
...
1. حدد **حفظ**. يؤدي هذا إلى إنشاء تمثيل JSON لسير العمل السحابي لمعالج الرسائل ويملأ حقل **بيانات العميل**.
1. حدد **حفظ وإغلاق**.
1. حدد **الاتصالات**. يجب أن ترى تعريفات مرجع اتصال **Microsoft Dataverse** و **Outlook.com** مدرجة. 

> [!NOTE]
> إذا رأيت تعريفات أخرى لمراجع الاتصال غير **Microsoft Dataverse** و **Outlook.com**، فهذا يعني أنك تستخدم أكثر من اتصال مرجع واحد في Dataverse في سير عمل معالج رسائل استلام الأمر. لحل هذه المشكلة، انتقل إلى سير عملك وتأكد من أن جميع إجراءات Dataverse تستخدم مرجع الاتصال نفسه.

## <a name="add-provider-definition-logic-definition-to-labproviders-solution"></a>إضافة تعريف منطق تعريف الموفر إلى حل LabProviders
